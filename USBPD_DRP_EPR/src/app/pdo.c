 ...
 */
static cy_en_pdstack_status_t is_rdo_valid (cy_stc_pdstack_context_t* context, cy_pd_pd_do_t rdo)
{
    uint32_t src_pdo_index;
    uint32_t rdo_obj_pos = rdo.rdo_gen.objPos;
#if (CY_PD_EPR_ENABLE)
    uint32_t rdo_epr_pdo = rdo.rdo_gen.eprPdo;
#endif /* CY_PD_EPR_ENABLE */
    cy_en_pdstack_status_t retval = CY_PDSTACK_STAT_FAILURE;
    cy_stc_pdstack_dpm_status_t *dpm = &(context->dpmStat);
    cy_stc_pdstack_dpm_ext_status_t *dpmExt = &(context->dpmExtStat);

    /* Validate the object position. */
#if (CY_PD_EPR_ENABLE)
    if(((rdo_epr_pdo == 0) && (rdo_obj_pos == 0))
    || ((rdo_epr_pdo == 0) && (rdo_obj_pos > dpm->curSrcPdocount))
    || ((rdo_epr_pdo != 0) && (rdo_obj_pos > dpmExt->curEprSrcPdoCount)))
#else
    if ((rdo_obj_pos == 0) || (rdo_obj_pos > dpm->curSrcPdocount))
#endif /* CY_PD_EPR_ENABLE */
    {
        return retval;
    }
...
#if (CY_PD_EPR_ENABLE)
    /* In request PDO index is SPR 1...7, EPR 8...13.
     * For SPR decrease index, to point in pdo list with 0...6.
     */
    if(rdo_epr_pdo != 0)
    {
        /* if bit31 is set in request, the index should pointing to epr part of pdo list.
         * i += 8, after decreasing the pointer is 7...12.
         */
        rdo_obj_pos += CY_PD_MAX_NO_OF_PDO + 1;
    }
#endif /* CY_PD_EPR_ENABLE */

    src_pdo_index = rdo_obj_pos - 1;

#if CY_PD_PPS_SRC_ENABLE || CY_PD_EPR_AVS_ENABLE
    if(dpm->curSrcPdo[src_pdo_index].src_gen.supplyType == CY_PDSTACK_PDO_AUGMENTED)
    {
        /* Augmented PDO cannot be matched in PD2.0 operation */
        if (context->dpmConfig.specRevSopLive <= CY_PD_REV2)
        {
            context->ptrAppCbk->app_event_handler(context, APP_EVT_BAD_SINK_APDO_SEL, NULL);
            return retval;
        }
#if CY_PD_EPR_AVS_ENABLE
        if(dpm->curSrcPdo[src_pdo_index].epr_avs_src.apdoType == CY_PDSTACK_APDO_AVS)
        {
            if(
                ((rdo.rdo_epr_avs.outVolt & 0x03) == 0) &&
                    Cy_PdStack_Dpm_IsValidAvsReq(context, src_pdo_index, rdo) == CY_PDSTACK_STAT_SUCCESS)
            {
                /* Set avs epr active flag */
                Cy_PdStack_Dpm_ChangeEprAvsActiveFlag(context, true);
                retval = CY_PDSTACK_STAT_SUCCESS;
            }
        }
#endif /* CY_PD_EPR_AVS_ENABLE */
#if CY_PD_PPS_SRC_ENABLE
        if(dpm->curSrcPdo[src_pdo_index].pps_src.apdoType == CY_PDSTACK_APDO_PPS)
        {
             /*  PPS Augmented PDO validation */
            if((dpm->curSrcPdo[src_pdo_index].pps_src.maxCur >=  rdo.rdo_pps.opCur) &&
               ((dpm->curSrcPdo[src_pdo_index].pps_src.maxVolt * 5u) >= rdo.rdo_pps.outVolt) &&
               ((dpm->curSrcPdo[src_pdo_index].pps_src.minVolt * 5u) <= rdo.rdo_pps.outVolt))
            {
                retval = CY_PDSTACK_STAT_SUCCESS;
            }
        }
    ...
    else
    {
        ...
        /* Check if RDO demands a valid power. */
        if ((rdo.rdo_gen.giveBackFlag != false) || (rdo.rdo_gen_gvb.capMismatch == true))
        {
            if (dpm->curSrcPdo[src_pdo_index].src_gen.maxCurPower >= rdo.rdo_gen.opPowerCur)
            {
                retval = CY_PDSTACK_STAT_SUCCESS;
            }
        }
        ...
        {
            if (dpm->curSrcPdo[src_pdo_index].src_gen.maxCurPower >= rdo.rdo_gen_gvb.maxPowerCur)
            {
                retval = CY_PDSTACK_STAT_SUCCESS;
            }
        }
...
#endif /* CCG_PPS_SRC_ENABLE || EPR_AVS_ENABLE */
...
void eval_rdo(cy_stc_pdstack_context_t* context, cy_pd_pd_do_t rdo, cy_pdstack_app_resp_cbk_t app_resp_handler)
{
    ...
    {
        app_get_resp_buf(context->port)->reqStatus = CY_PDSTACK_REQ_SEND_HARD_RESET;
        app_resp_handler(context, app_get_resp_buf(context->port));
        return;
    }
#endif /* CY_PD_REV3_ENABLE && (!CY_PD_EPR_ENABLE) */

    if (is_rdo_valid(context, rdo) == CY_PDSTACK_STAT_SUCCESS)
    {
        app_get_resp_buf(context->port)->reqStatus = CY_PDSTACK_REQ_ACCEPT;
    }
    else
    {
        app_get_resp_buf(context->port)->reqStatus = CY_PDSTACK_REQ_REJECT;
    }
